...
function onOpen() {
  ...
  // needs speed
  writeBlock(gAbsIncModal.format(96), "S100");
  ...
  // Feed
  writeBlock(feedOutput.format(2000));
}
...
function setDeviceMode(enable) {
  if (enable != deviceOn) {
    deviceOn = enable;
    if (enable) {
      writeComment("LOWER HEAD AND TURN ON CUTTING");
      writeBlock(mFormat.format(8));
      writeBlock(gFormat.format(4), "P0.2");
      writeBlock(mFormat.format(3));
      writeBlock(gFormat.format(4), "P0.6");
    } else {
      writeComment("TURN OFF CUTTING AND RAISE HEAD");
